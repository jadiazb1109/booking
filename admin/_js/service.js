...
document.addEventListener('DOMContentLoaded', () => {
    ...
    var txtOpcion = "";
    var urlApi = "../../api/v1/service/services";
            ...
            if (selectId > 0) {
                ...
                $.ajax({
                    ...
                    if (resultado["state"] === 'ok') {
                        ...
                        clsGenerales_.fnComponetInstanceSetValue(cmbType, resultado["data"][0].type_id);
                        txtDescripcion.value = resultado["data"][0].service;
                        txtNotes.value = resultado["data"][0].notes;
                        chkReturn.checked = resultado["data"][0].return ? true : false;
                        chkNumberRoom.checked = resultado["data"][0].room_number ? true : false;
    ...
    const chkNumberRoom = document.getElementById('chkNumberRoom');
    const chkActivo = document.getElementById('chkActivo');
    const btnGuardar = document.getElementById('btnGuardar');
    const btnCancelar = document.getElementById('btnCancelar');

    const cmbType = $('#cmbType').dxSelectBox({
    ...
    clsGenerales_.mtdLlenarComponetTipos("../../api/v1/service/typeActive", cmbType);
    ...
    function clearInputs() {    
        clsGenerales_.fnComponetInstanceSetValue(cmbType, null);  
        txtDescripcion.value = null;
        txtNotes.value = null;
        chkReturn.checked = false;
        chkNumberRoom.checked = false;
        chkActivo.checked = true;        
    }
    ...
    function btnGuardarClick(e) {
        ...
        if (!clsGenerales_.fnComponetInstanceGetValue(cmbType)) {
            clsGenerales_.mtdMostrarMensaje("Select a type", "error");
            cmbType.focus();
            return;
        }
        ...
        if (txtOpcion == "Adicionar" || txtOpcion == "Modificar") {
            ...
            $.ajax({
                ...
                headers: { "Api-Key": clsGenerales_.obtenerAPI_KEY() },
                data: JSON.stringify({
                    type_id: clsGenerales_.fnComponetInstanceGetValue(cmbType),
                    name: txtDescripcion.value,
                    notes: txtNotes.value,
                    return: chkReturn.checked,
                    room_number: chkNumberRoom.checked,
